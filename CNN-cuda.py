...
debug = False # depend on the information to be printed or writed in log_file
...
try:
    ...
    def hinge_loss(s1,s2,t0,batch_size):
        ...
        if cuda_able:
            loss = Variable(torch.Tensor(1)).cuda()
        else:
    ...
    #         x3 = self.pool3(F.tanh(self.conv3(x3)))
            ...
            return hinge_loss(pos_cosine, neg_cosine, t0, batch_size), pos_cosine, neg_cosine
    net = Net()
    if cuda_able:
        net.cuda()
